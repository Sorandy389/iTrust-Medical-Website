...
/*new added table for obstetrics*/

CREATE TABLE obstetricsrecord(
  ID INT UNSIGNED AUTO_INCREMENT primary key,
  patientID BIGINT unsigned NOT NULL default '0',
  createdDate varchar(30),
  LMP varchar(30),
  yearOfConception INT NOT NULL,
  numberOfWeeksPreg varchar(4) NOT NULL,
  numberOfLaborHour double DEFAULT 0,
  weightGain double Default 0,
  deliveryType enum('vaginal delivery', 'vaginal delivery vacuum assist', 'vaginal delivery forceps assist', 'caesarean section', 'miscarriage','N/A') NOT NULL,
  numberofBaby int DEFAULT 0,
  FOREIGN KEY (patientID) REFERENCES patients(MID)
)ENGINE=MyISAM;

/*Data table for obstetrics Office Visit*/
...
CREATE TABLE obstetricsofficevisit(
  ...
  locationID VARCHAR(10) NOT NULL,
	...
	notes TEXT,
	sendBill BOOLEAN default false,
  LMP DATETIME NOT NULL,
  weightInPounds double Default 0,
  bloodPressure VARCHAR(7),
  FHR int DEFAULT 0,
  numberOfBaby int DEFAULT 0,
  lowLyingPlacenta BOOLEAN default false,
...
CREATE TABLE ultrasoundrecord(
  ID INT UNSIGNED AUTO_INCREMENT primary key,
  patientID BIGINT unsigned NOT NULL default '0',
  visitID BIGINT unsigned NOT NULL default '0',
  createdDate DATETIME NOT NULL,
  URL TEXT,
  CRL double DEFAULT 0,
  BPD double DEFAULT 0,
  HC double DEFAULT 0,
  FL double DEFAULT 0,
  OFD double DEFAULT 0,
  AC double DEFAULT 0,
  HL double DEFAULT 0,
  EFW double DEFAULT 0,

  FOREIGN KEY (patientID) REFERENCES patients(MID),
  FOREIGN KEY (visitID) REFERENCES obstetricsofficevisit(ID)
)ENGINE=MyISAM;

CREATE TABLE ultrasoundfile(
  ID INT UNSIGNED AUTO_INCREMENT primary key,
  recordID BIGINT unsigned NOT NULL default '0',
  contents longblob,
  filename TEXT,
  FOREIGN KEY (recordID) REFERENCES ultrasoundrecord(ID)
)ENGINE=MyISAM;
...
CREATE TABLE patients(
  obStatus BOOLEAN default false,
...
CREATE TABLE IF NOT EXISTS obstetricsrecord  (
  ID INT UNSIGNED AUTO_INCREMENT primary key,
  patientID BIGINT unsigned NOT NULL default '0',
  createdDate varchar(30),
  LMP varchar(30),
...
CREATE TABLE childhospitalvisit
(
	id 						BIGINT(20) 		UNSIGNED AUTO_INCREMENT,
	apptID				INT UNSIGNED,
	patientID     BIGINT(20)    UNSIGNED NOT NULL,
	preferredDeliveryType 				enum('vaginal delivery', 'vaginal delivery vacuum assist', 'vaginal delivery forceps assist', 'caesarean section', 'miscarriage','N/A') NOT NULL,
  actualDeliveryType 		enum('vaginal delivery', 'vaginal delivery vacuum assist', 'vaginal delivery forceps assist', 'caesarean section', 'miscarriage','N/A') NOT NULL,
	pitocin      INT(20),
	nitrousOxide      INT(20),
	pethidine      INT(20),
	epiduralAnaesthesia      INT(20),
	magnesiumSulfate      INT(20),
	RHImmuneGlobulin     INT(20),
...
(
  id     BIGINT(20) 		UNSIGNED AUTO_INCREMENT,
  childhospitalvisitID   BIGINT(20) NOT NULL,
  sex        enum('MALE', 'FEMALE'),
  patientID    BIGINT(20)    UNSIGNED NOT NULL,
  motherID    BIGINT(20)    UNSIGNED NOT NULL,
  deliverTime VARCHAR(30),
  deliverDate VARCHAR(30),

  PRIMARY KEY (id),
  FOREIGN KEY (childhospitalvisitID) REFERENCES childhospitalvisit(id),
  FOREIGN KEY (patientID) REFERENCES patients(MID) ON DELETE CASCADE
) ENGINE=MyISAM;