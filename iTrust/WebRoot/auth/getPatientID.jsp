...
<%
	...
	if(lastName == null)
		lastName = "";
	String obstetric = request.getParameter("isObstetric");
	String CHV = request.getParameter("isCHV");
	String LDR = request.getParameter("isLDR");


	boolean isAudit = request.getParameter("forward") != null && request.getParameter("forward").contains("hcp/auditPage.jsp");

	boolean isCHV = request.getParameter("forward") != null && CHV != null && CHV.equals("true");
	// new added
	boolean isLDR = request.getParameter("forward") != null && LDR != null && LDR.equals("true");

	boolean isObstetric = false;
	if(request.getParameter("forward") != null) {
	    String forwardUrl = request.getParameter("forward");
	    String[] obsUrls = new String[] {"hcp-uap/viewObstetricsRecords.jsp" , "hcp-uap/viewObsVisit.jsp" , "hcp-uap/viewChildHospitalVisit.jsp","hcp-uap/viewLaborAndDelivery.jsp"};
	    for(String url: obsUrls) {
			isObstetric = isObstetric || forwardUrl.contains(url);
		}
	}
%>
...
<script>
function confirmPatient(pid, forward) {
    var r = confirm("Are you sure to choose this patient [" + pid + "]?");
    if (r) {
        parent.location.href = forward + "?pid=" + pid;
    }

}

function notEligiPatient() {
	alert("Please Select Obstetric Eligible Patient!");
	location.reload();
}
</script>
