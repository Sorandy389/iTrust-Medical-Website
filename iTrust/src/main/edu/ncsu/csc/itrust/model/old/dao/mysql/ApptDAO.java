...
public class ApptDAO {
	...
	public List<ApptBean> getAllApptsForPatient(long mid, long pid) throws SQLException, DBException {
		try (Connection conn = factory.getConnection();
				PreparedStatement stmt = 
			conn.prepareStatement("SELECT * FROM appointment WHERE doctor_id=? AND patient_id=? ORDER BY sched_date;")) {
			stmt.setLong(1, mid);
			stmt.setLong(2, pid);
