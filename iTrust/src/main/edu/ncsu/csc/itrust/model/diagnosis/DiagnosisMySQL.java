...
public class DiagnosisMySQL implements DiagnosisData {
	 ...
	 * {@inheritDoc}
	 */
	public List<Diagnosis> getByVisitID(long visitId) throws DBException {
		try {Connection conn = ds.getConnection();
				PreparedStatement ps = conn.prepareStatement("SELECT * FROM diagnosis, icdcode where visitId = ?");
				ps.setLong(1, visitId);
				ResultSet rs = ps.executeQuery();
			return rs.next() ? loader.loadList(rs) : null;
		} catch (SQLException e) {
			throw new DBException(e);
		}
	}
