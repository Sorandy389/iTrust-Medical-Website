...
public class PatientDAO {
	...
	public boolean getObStatus(long mid) throws ITrustException, DBException{
		try (Connection conn = factory.getConnection();
			 PreparedStatement ps = conn.prepareStatement("SELECT * FROM patients WHERE MID=?")) {
			ps.setLong(1, mid);
			ResultSet rs = ps.executeQuery();
			if (rs.next()){
				return rs.getBoolean("obStatus");
			}
	...
	public void setObStatus(long mid, boolean flag) throws ITrustException, DBException{
		try (Connection conn = factory.getConnection();
			 PreparedStatement ps = conn.prepareStatement("UPDATE patients SET patients.obStatus=? WHERE MID=?")) {
